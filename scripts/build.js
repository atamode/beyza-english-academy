...
function walk(dir){return fs.existsSync(path.join(root,dir))?fs.readdirSync(path.join(root,dir),{withFileTypes:true}).flatMap(e=>{const p=path.join(dir,e.name).replace(/\\/g,"/");return e.isDirectory()?walk(p):[p]}):[]}
...
base.splice(base.indexOf("js/student-repository.js"), 0, "js/payment-service.js");
