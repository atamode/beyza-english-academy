...
export function storyCenterCard({ home = false } = {}) {
  return `<section class="card game-center-card story-center-card ${home ? "home-game-card" : ""}" data-game-center="stories"><div class="game-card-layout"><img class="game-card-image" src="assets/stories/story-001/story-001-01-village-empty-master-1280x720.webp" alt="Pomante Krallığı kale kapısı"><div><p class="eyebrow">HİKÂYE</p><h2>Poma Hikâyeleri</h2><p class="lead">Pomante Krallığı'nda İngilizce hikâyeleri keşfet.</p><div class="button-row"><button class="button ${home ? "secondary" : "primary"}" data-route="stories">${home ? "Oku" : "Hikâyeleri Aç"}</button></div></div></div></section>`;
}
...
function mediaAlt(id) {
  return ({
    "kingdom-empty": "Pomante Krallığı kale kapısı",
