...
test("31 öğretmen iade butonu yalnız aktif ve kullanılmamış kredi için görünür",()=>{assert.match(service,/teacher_access_credits\(id,status,starts_at,ends_at\)/);assert.match(views,/teacherEligible=credit\?\.status==="active"/);assert.match(views,/credit\.ends_at/);assert.match(views,/credit\.starts_at/);});
test("32 admin iade kartı ödenen ve iade tutarlarını ayrı gösterir",()=>{assert.match(views,/Ödenen: \$\{money\(row\.payable_amount\)\}/);assert.match(views,/İade: \$\{money\(row\.refund_amount\)\}/);});
test("33 admin inceleme ve tamamlanma zamanlarını görür",()=>{assert.match(service,/refund_requests"\)\.select\("id,reviewed_at"\)/);assert.match(views,/refund_reviewed_at/);assert.match(views,/refund_completed_at/);});
test("34 muhasebe uyarısı admin RLS okumasıyla payout ve komisyon bağını gösterir",()=>{assert.match(service,/refund_accounting_alerts"\)\.select\("id,refund_request_id,payment_request_id,commission_earning_id,payout_id,commission_amount,alert_type,status,resolution_note,resolved_at"\)/);for(const field of ["commission_amount","commission_earning_id","payout_id","alert_type","resolution_note"])assert.match(views,new RegExp(field));});
test("35 açık ve çözülmüş muhasebe durumları Türkçedir",()=>{assert.match(views,/latestAlert\?\.status==="resolved"\?"Çözüldü":"Açık"/);});
