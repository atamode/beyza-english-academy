...
test("clipboard failure uses a safe fallback without locking payment flow",()=>{assert.match(entry,/navigator\.clipboard\?\.writeText/);assert.match(entry,/document\.execCommand\("copy"\)/);assert.match(entry,/catch \{ return false; \} finally \{ field\?\.remove\(\); \}/);assert.doesNotMatch(entry,/copy-(?:iban|payment-code)[\s\S]{0,500}button\.disabled/);});
