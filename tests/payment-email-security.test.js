...
test("admin retry states and SECURITY DEFINER count remain constrained",()=>{
  ...
  assert.match(matrix,/SECURITY DEFINER toplamı \| 58/);assert.match(matrix,/transactional e-posta outbox/);assert.match(matrix,/yeni bir SECURITY DEFINER fonksiyon eklemez/);
});
